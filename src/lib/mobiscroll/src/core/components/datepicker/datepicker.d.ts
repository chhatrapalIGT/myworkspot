...
export interface MbscDatepickerOptions extends MbscCalendarOptions, MbscDatetimeOptions {
    controls?: Array<'calendar' | 'date' | 'time' | 'datetime' | 'timegrid'>;
    clearIcon?: string;
    defaultValue?: any;
    rangeHighlight?: boolean;
    rangeSelectMode?: 'cycle' | 'wizard';
    modules?: any[];
    tabs?: boolean | 'auto';
...
export declare class DatepickerBase extends PickerBase<MbscDatepickerOptions, MbscDatepickerState> {
    ...
    /** It's a Flag that is set to 'start' or 'end' when one of the controls should be focused after render.
     * It holds which control (start/end) should be focused in the update lifecycle when using the keyboard as navigation.
     */
     ...
     *    if the labels are not shown, we allow the selection in the case of date control - there's no way to
     *    switch to second value otherwise
     */
