...
function Profile({
  ...
  useEffect(() => {
    ...
    // Remove the problematic call - this logic should be handled elsewhere
    // The badge verification should happen on input change, not in useEffect
  }, [show]);
  useEffect(() => {
    if (validateBadge) {
      setOpenBadge(true);
  ...
  const handleSpinRemove = (name) => {
    ...
    setemployee((prestate) =>
      prestate.filter((ele) => ele.id !== dataVal[0].id),
    );
  ...
  return (
    <div>
      {(apiMessage || locationMessage || locationApiMessage || empSuccess) && (
        <div
      ...
      >
                            ...
                            <div className="d-flex">
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  // Check if both inputs are complete and match
                                  const newBadgeVerify =
                                    e.target.value.concat(inputSet2);
                                  if (
                                    newBadgeVerify.length === 6 &&
                                    newBadgeVerify === badgeConfirmVerify
                                  ) {
                                    const mockEvent = {
                                      target: {
                                        name: 'badge',
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  // Check if both inputs are complete and match
                                  const newBadgeVerify = inputSet.concat(
                                    e.target.value,
                                  );
                                  if (
                                    newBadgeVerify.length === 6 &&
                        ...
                        <>
                          ...
                          `}
                            ...
                            <div className="action-buttons">
                              ...
                              >
                                Save
                                {(verifyBadgeLoading || badgeUpdateLoading) && (
        ...
        <div className="workspot-access mt-40">
          <div className="container">
            <h4 className="common-title">ERG Pins</h4>
            <p className="w-50 stroke-2 mt-3">
              You can select up to 3 pins to display.
            </p>
            {isLoading ? (
              <div className="card mt-4 weekly-default-inner d-flex flex-wrap">
                <Spinner
                  ...
                  {selectEmpIcon &&
                    ...
                    selectEmpIcon.map((i) => (
                      <div className="access-one">
                        <img
                          src={`${
                            SPIN_IMAGE_URL_LIVE + i.imageUrl
        ...
        <div className="workspot-access mt-40">
          <div className="container">
            ...
            <p className="stroke-2 mt-3">
              ...
              to the office manager's team. It may take up to 48 business hours
                ...
                {searchName &&
                  searchName.map((i) => (
                    <div
                      aria-hidden="true"
                      className={`${
                        selectData.includes(i) && 'checked_item'
                      }  form-group`}
      ...
      >
        <div className="modal-header d-block">
          ...
          <div className="modal-body modal-update margin_spin" id="data_update">
            <form className="delegate-workspot-access" action="submit">
              <div>
                {spinIcon &&
                  spinIcon.length > 0 &&
                  spinIcon.map((elec) => (
                    <div
                      aria-hidden="true"
                      className={`${
                        demoData.includes(elec.id) && 'checked_item'
                      } form-group`}
                      onClick={() => handleSpinSelect(elec)}
                    >
                      <img
                        src={`${
                          SPIN_IMAGE_URL_LIVE + elec.imageUrl
                        }?bust=${new Date().getTime()}`}
                        alt=""
                      />
                      <input
